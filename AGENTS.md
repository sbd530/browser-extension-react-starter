...
Conventions for AI agents (Claude Code, Cursor, Copilot, etc.) working in this
...
- Use **Node 24**. The repo pins it in `.node-version` / `.nvmrc`. With `fnm`,
  prefix commands as `fnm exec --using=24 -- <cmd>` if your shell is not already on 24.
...
```
entrypoints/   extension entrypoints (popup, options, sidepanel, newtab,
               devtools, devtools-panel, background, content, content-ui, content-runtime)
...
public/        copied verbatim (icons, vendored Geist fonts)
...
- **Comments and docs are English. Keep comments minimal.** Chat with the maintainer
  in Korean.
- **Path alias `@/`** maps to the project root (provided by the generated tsconfig).
- Add shadcn components with the CLI: it needs a Vite project, which WXT hides, so
  create a temporary `vite.config.ts` (plugins: react + tailwindcss, alias `@`), run
  `npx shadcn@latest add <name>`, then delete the temp file.

## Theming

- One source of truth: the `local:theme` storage item (`lib/theme.ts`), so the choice
  is shared across every page and context.
- `ThemeProvider` (`components/theme-provider.tsx`) applies `.dark` to
  `document.documentElement` for pages. Content-script UIs apply it to their shadow-root
  wrapper via `lib/content-theme.ts`.
- Design tokens live in `assets/theme.css`, scoped to `:root, :host` so they also work
  inside content-script shadow roots.

## i18n

- `@wxt-dev/i18n`. Source in `locales/<locale>.yml`; default locale `en`.
...
tag `X.Y.Z`, and publish a GitHub Release. App version starts at `0.1.0`. The local git
identity for this repo is `sbd530` / `byungdon92@gmail.com`.
...
`AGENTS.md`, `.claude/`). Alternatively, keep these files only on a dedicated branch so
the default branch stays clean for downstream projects.
